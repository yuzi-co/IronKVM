...
//
// The rebuild drops the write lock before reading rather than holding it across
// the read. read() is a cgo call that takes libkvm's own mutex and runs for up
// to a frame time, and holding this lock exclusively for that long would stall
// every other viewer. The gap it leaves is a stop landing between the rebuild
// and the read, which returns false. That costs one frame: every caller is a
// streamer that treats a missing frame as "none this time" and asks again.
