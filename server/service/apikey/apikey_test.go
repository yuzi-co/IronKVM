...
// owner is the account the tests issue keys to.
const owner = "admin"
...
func TestUnknownKeyIsRejected(t *testing.T) {
	...
	if _, _, err := Create("automation", owner); err != nil {
...
func TestRevokedKeyStopsWorking(t *testing.T) {
	...
	kept, _, err := Create("kept", owner)
	if err != nil {
		t.Fatalf("setup: %s", err)
	}

	revoked, record, err := Create("revoked", owner)
	if err != nil {
		t.Fatalf("setup: %s", err)
	}

	if err := Revoke(record.ID, owner); err != nil {
		t.Fatalf("expected the key to be revoked: %s", err)
	}

	if _, ok := Verify(revoked); ok {
		t.Fatal("expected the revoked key to stop working")
	}

	if _, ok := Verify(kept); !ok {
...
func TestOneOperatorDoesNotSeeAnothersKeys(t *testing.T) {
	withTempKeyStore(t)
	...
	if len(keys) != 0 {
		t.Fatalf("listed %d keys of another account, want 0", len(keys))
	}
}

func TestOneOperatorCannotRevokeAnothersKey(t *testing.T) {
	withTempKeyStore(t)
	...
	if err := Revoke(record.ID, owner); err != ErrNotFound {
		t.Fatalf("revoking another account's key returned %v, want ErrNotFound", err)
	}

	if _, ok := Verify(secret); !ok {
		t.Fatal("the key must survive a revoke by an account that does not hold it")
	}
}

func TestAKeyWithNoAccountGoesToTheSystemAccount(t *testing.T) {
	// Keys issued before the device had more than one account name nobody.
	// They belong to the account that owned it then, and a device that cannot
	// say which account that was must not hand them an identity.
	path := withTempKeyStore(t)

	secret, _, err := Create("legacy", "")
	if err != nil {
		t.Fatalf("setup: %s", err)
	}

	original := systemAccount
	t.Cleanup(func() { systemAccount = original })

	systemAccount = func() string { return "owner" }
	key, ok := Verify(secret)
	if !ok || key.Username != "owner" {
		t.Fatalf("adopted key = %+v, ok=%v, want it to belong to owner", key, ok)
	}

	systemAccount = func() string { return "" }
	key, ok = Verify(secret)
	if !ok || key.Username != "" {
		t.Fatalf("unplaceable key = %+v, ok=%v, want it to name no account", key, ok)
	}
