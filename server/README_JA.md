...
jwt:
   # JWT 秘密鍵の設定。 空のままにすると、サーバーは初回起動時にランダムな 64 バイトの鍵を生成し、`/etc/kvm/.jwt_secret` に保存します。 再起動してもセッションは有効なままです。
   secretKey: ""
...
# ION 予約領域
ion:
   # ストリームの開始が必要とするメモリ量の推定値です（バイト単位）。 サーバーは動作中に実際のコストを測定します。 この値と実測値のうち大きい方を使用します。 デフォルト: 12582912 (12MB)
