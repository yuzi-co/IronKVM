...
typedef struct {
    ...
    //
    // Both threads test try_exit_thread once per pass, so a join costs one
    // pass. The watchdog polls every 500ms. vi_subsystem_detection tests it at
    // the top of its loop, so the bound holds only while everything the loop
    // calls returns to it - which is why the probe fix has to come first.
...
void kvmv_deinit()
{
    ...
    // apart. Each join costs one pass of the thread's loop; the watchdog polls
    // every 500ms, and the detection thread reaches its own try_exit_thread
    // test as long as the resolution probe returns to it.
    //
