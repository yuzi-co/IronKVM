...
# Pull the two blocks back out of the init that was actually built, and run the
# scenarios against those. Testing the .inc files again would only prove the
# inputs are good; this proves the splice put them in, in one piece, in the
# right order. A repacker that dropped a block would otherwise ship an image
# whose sources all passed their tests.
sed -n '/^# --- slot selection ---$/,/^# --- end slot selection ---$/p' \
    "$OUT/tree.chk/init" > "$OUT/spliced-selection.inc"
sed -n '/^# --- slot dispatch ---$/,/^# --- end slot dispatch ---$/p' \
    "$OUT/tree.chk/init" > "$OUT/spliced-dispatch.inc"

if [ ! -s "$OUT/spliced-selection.inc" ] || [ ! -s "$OUT/spliced-dispatch.inc" ]; then
    note "both slot blocks are present in the built init" FAIL
else
    note "both slot blocks are present in the built init" OK
    if sh "$HERE/test-dispatch.sh" "$OUT/spliced-selection.inc" "$OUT/spliced-dispatch.inc"; then
...
# The selection block must come before the dispatch block, because it reads
# /boot while it is still mounted and the dispatch runs after it is not.
L_SEL=$(grep -n '^# --- slot selection ---$' "$OUT/tree.chk/init" | head -1 | cut -d: -f1)
L_DIS=$(grep -n '^# --- slot dispatch ---$'  "$OUT/tree.chk/init" | head -1 | cut -d: -f1)
if [ -n "$L_SEL" ] && [ -n "$L_DIS" ] && [ "$L_SEL" -lt "$L_DIS" ]; then
    note "selection is spliced before dispatch" OK
else
    note "selection at ${L_SEL:-none}, dispatch at ${L_DIS:-none}" FAIL
fi
