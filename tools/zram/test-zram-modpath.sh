...
#
# zsmalloc.ko and zram.ko are built out of tree and were copied to
# /mnt/system/ko by hand. /mnt is not a mount point. It is a plain directory on
# the root slot, so a slot re-flash or a rootfs rebuild deletes both modules,
# and the only trace is one `S01zram (rc=1)` line in /bootlog. Observed on the
# reference board on 2026-08-19: zram had been off since a rebuild three days
# earlier and nothing reported it.
#
# The repair is to stop treating the modules as something a person installs.
# /kvmapp/system/ko already ships in the install package, and already carries
# two other modules, so an image built from this repository carries these two
# as well and an over-the-air update restores them.
#
# /mnt/system/ko stays in the list, second. A device that was set up by hand
# before this change keeps the modules it has, and a base image that ever ships
# its own pair keeps working.
#
